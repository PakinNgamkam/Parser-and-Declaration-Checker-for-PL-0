...
// ****** const_defs****** //
void scope_check_constDefs(const_defs_t cdefs)
{
    const_def_t *cdefp = cdefs.const_defs;
    while (cdefp != NULL) {
	scope_check_constDef(*cdefp);
	cdefp = cdefp->next;
    }
}

// ****** const_def****** //
void scope_check_constDef(const_def_t cdef)
{
    const char *name = cdef.ident.name;
    id_use *idu = scope_check_ident_declared(*(cdef.ident.file_loc),
					     name);
...
void scope_check_procDecl(proc_decl_t pd)
{
    ...
    id_use *idu = scope_check_ident_declared(*(pd.file_loc),
					     name);
    assert(idu != NULL);  // since would bail if not declared
